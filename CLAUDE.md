...
**Hooks reach a project two ways, and the script tells them apart by its own location.** A plugin group ships `hooks/hooks.json`, which Claude Code activates whenever that plugin is enabled, anchored to `${CLAUDE_PLUGIN_ROOT}`. That copy is live in *every* project, so an opinionated hook like `block-dashes` stays dormant until the project opts in by creating `.claude/no-dashes`. The other way is a project-local install (`.claude/hooks/<name>.py` plus a `settings.json` entry), which `forge-adapt` still does for a cloned library; a copy under the project root is itself the opt-in and needs no sentinel. `enforcement_enabled()` distinguishes them by testing whether the script resolves inside the project root, which keeps pre-existing installs working and avoids depending on which environment variables happen to be exported. Prefer plugin registration: it owns no user config, so there is no wiring to drift, duplicate, or clobber. That copy-and-mutate path was the origin of every hook bug in this repo's history.
