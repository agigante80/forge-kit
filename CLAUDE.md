...
**forge-kit** is an AI-assisted project governance scaffold: AI-agnostic at the governance layer (issue templates, labels, GWT scenarios), Claude Code-native at the automation layer (agents, skills, slash commands). It is a template repository, not a buildable application. Its purpose is to be bootstrapped into other projects or used as an upgrade reference via the `forge-adapt` skill. There are no build steps or package managers. The only CI is a governance `Validate` workflow (`.github/workflows/validate.yml`): it runs the structural check, the template lockstep, every contract test suite under `scripts/test-*`, the component-index freshness check, the component size budget, the two range guards (on `pull_request` AND `push`, #158), plus one advisory `claude plugin validate` step marked `continue-on-error` that can report issues without failing the build. There is no application build/test pipeline. Some of those suites cover shipped executables (among them hooks, the catalogue script, the agent-skills script, `forge-lib.sh` twice, the component-index generator, both leak scanners, the two roadmap assets, the gate round counter, the install planner and the drift reporter); the rest cover the repo's own guards (among them `test-validate-plugins.sh`, `test-check-reference-depth.sh`, `test-check-neighbour-overlap.sh`, `test-forge-adapt-neighbour-disposition.sh`, `test-template-lockstep.sh`, `test-check-plugin-version-bump.sh`, `test-component-size.sh`, `test-pre-push-hook.sh`, `test-pre-commit-hook.sh`, `test-component-paths.sh`, `test-check-restatements.sh`, `test-producer-stamps.sh`, `test-template-dir-order.sh`, `test-check-group-isolation.sh`, `test-resolve-range-base.sh`, `test-check-live-placeholders.sh`, `test-check-component-scope.sh`, `test-check-lens-contract.sh`), and all of them run unconditionally.
...
1. **Structural / discipline checks** (the same gates CI runs; run these before committing):
   ...
   bash scripts/check-template-dir-order.sh    # fail if any copy of the template-dir order disagrees
   ...
   bash scripts/test-update-suite-counts.sh     # contract test for the suite-count generator
   python3 scripts/update-suite-counts.py --check  # fail if a `scripts/test-X`, N tests count below is stale
...
2. **Behavioural validation:** agents, skills, and commands are prose, cannot be run in isolation here, and must be installed into a test project via `forge-adapt` and exercised there. **The exception is anything the kit ships as an executable**, which has a real contract and therefore a real test. Every one of them is contract-tested in CI, each exercised as a subprocess (throwaway directories, this repo itself, or a stubbed transport, per bullet):
   ...
   - **`scripts/forge-adapt-agent-skills.sh`** (`scripts/test-forge-adapt-agent-skills.sh`, 45 tests, in CI): resolves the companion skills an agent declares in its `skills:` frontmatter, and rewrites the plugin scope to the bare project name at install time. A script for the same reason the catalogue is one, and because the failure it prevents is SILENT: a declared skill that is missing is skipped with only a debug-log warning, so the agent runs with its reference material gone and the project shows no error. It parses two YAML SHAPES rather than YAML, and REFUSES anything else (a multi-line flow list, a plain scalar) instead of guessing, because a half-understood rewrite leaves frontmatter that no longer parses and the agent then stops loading entirely, which is worse than the bug it was avoiding.
   - **`scripts/update-suite-counts.py`** (`scripts/test-update-suite-counts.sh`, 47 tests, in CI): rewrites every `` `scripts/test-X`, N tests `` count in this file from the total the suite PRINTS, and `--check` fails the build when one is stale (#201). **Do not hand-edit those numbers**; run the script. `N` is the printed pass-plus-fail total, which counts assertions rather than case sections, because that is the one number a suite reports for itself. The anchor is the prose shape itself, with no inline markers: the backticked path, a comma, the digits, the word `tests`, each gap allowed one line break; a later `N tests` in the same bullet is prose and is never touched. It reads the combined stdout and stderr stream (unittest reports on stderr) and takes the LAST matching line, and a suite that is missing or prints no recognisable total REFUSES with exit 2 rather than writing a zero. Three numbers in this file were stale on its first run, on top of the two the gate had already caught, which is the argument for it.
   ...
   - **`leak-guard/assets/check-public-leaks.sh`** (`scripts/test-check-public-leaks.sh`, 179 tests,
...
point at the good path, and after #163 that is the case the tree can prove. Every component
is user-scoped today. `scripts/check-component-scope.sh` prints the count, reads FRONTMATTER only (a
