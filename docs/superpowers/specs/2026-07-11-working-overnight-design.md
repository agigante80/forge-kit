...
|---|---|---|
| Substrate | A single long live session, armed before the user leaves | User choice. Simplest to launch; its weakness (context exhaustion) is compensated by the orchestrator-delegates-to-subagents pattern and a mandatory backstop. |
...
| Home and enforcement | A `working-overnight` skill in forge-kit-governance plus an `overnight-continue` Stop hook | User choice. The hook is what stops the session halting on a question; the skill holds the judgment. |
...
The session runs as a lean orchestrator: real work is delegated to subagents so
their tool output consumes their context, not the orchestrator's, which keeps the
live session alive far longer.
...
A Stop hook that fires when the agent finishes a turn. Its job is to prevent an
armed overnight run from idling to a stop on a question.

- Sentinel-gated: it acts only when `.claude/overnight/active.md` exists. With no
  manifest it does nothing, so normal daytime sessions are never affected. This
  mirrors how `block-dashes` stays dormant without `.claude/no-dashes`.
- Fails OPEN as allow-stop. This is the inverse of a guard hook. For an
  ...
  unparseable payload, or missing/short manifest must let the session stop. A
  broken hook must never be able to loop a session forever.
- Respects wind-down: once the skill removes the sentinel, the next Stop is
  allowed. The hook enforces "do not idle-stop"; the skill owns "when to stop".

When armed and healthy, the hook returns a decision that continues the session
with a reason pointing back at the working-overnight loop.
...
- Single-session context exhaustion is mitigated (orchestrator plus subagents,
  output redirection, per-loop rule refresh, low-context backstop), not
  eliminated. The backstop is mandatory, not optional.
...
- The `overnight-continue` hook gets contract tests in `scripts/test-hooks.py`:
  armed manifest present -> continue; no manifest -> allow stop; malformed payload
  -> allow stop (fail-open); sentinel removed mid-run -> allow stop.
