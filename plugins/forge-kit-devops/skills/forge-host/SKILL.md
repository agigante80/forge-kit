...
| `forge_issue_comment <n> <body>` / `forge_issue_close <n>` / `forge_issue_edit <n> <body>` | act on issues (edit REPLACES the body and refuses an empty one). Silent on success; on a Forgejo 404 each prints one stderr line naming the function and the issue and returns 44 (#229; on GitHub `gh` prints its own line and the code is `gh`'s), so check the exit code, never the silence |
...
4. Components adopt the adapter by replacing direct `gh` calls with `forge_*`; see
   `references/adopting-forge-lib.md` for the per-component swaps, and `references/forgejo.md`
