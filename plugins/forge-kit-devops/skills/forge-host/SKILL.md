...
| `forge_issue_comment <n> <body>` / `forge_issue_close <n>` / `forge_issue_edit <n> <body>` | act on issues (edit REPLACES the body and refuses an empty one). Silent on success; on a 404 each prints one stderr line naming the function and the issue and returns 44 (#229), so check the exit code, never the silence |
